...
private:
    ...
    ros::Publisher social_heatmap_pub_;
    ...
    std::string map_frame_,
        fixed_frame_, robot_frame_, odometry_topic_, global_2d_map_topic_, social_agents_topic_, social_heatmap_topic_, octomap_service_;
    ...
    double octree_resol_, minimum_range_, rviz_timer_, robot_distance_view_, robot_velocity_thres_, robot_fov_, social_heatmap_decay_factor_;

    int social_heatmap_resolution_factor_;

    // Point Clouds
    std::vector<std::string> point_cloud_topics_, point_cloud_frames_;

    //  global map
    nav_msgs::OccupancyGrid global_2d_map_;

    // odometry variable
    nav_msgs::Odometry robot_odometry_;

    // agent states
    pedsim_msgs::AgentStates agent_states_;

    // social costmap
    SocialHeatmap social_heatmap_ = SocialHeatmap();
    nav_msgs::OccupancyGrid current_social_heatmap_;
      ...
      rviz_timer_(0.0)
{
    ...
    local_nh_.param("social_heatmap_topic", social_heatmap_topic_,
                    social_heatmap_topic_);
    local_nh_.param("social_heatmap_decay_factor", social_heatmap_decay_factor_,
                    social_heatmap_decay_factor_);
    local_nh_.param("social_heatmap_resolution_factor", social_heatmap_resolution_factor_,
                    social_heatmap_resolution_factor_);
    ...
    social_heatmap_pub_ = local_nh_.advertise<nav_msgs::OccupancyGrid>(social_heatmap_topic_, 1, true);
    ...
    social_heatmap_.setResolutionFactor(social_heatmap_resolution_factor_);
    social_heatmap_.setDimensions(global_2d_map_.info.width, global_2d_map_.info.height);
    social_heatmap_.setFrameId(global_2d_map_.header.frame_id);
    social_heatmap_.setOrigin(global_2d_map_.info.origin);
    social_heatmap_.setResolution(global_2d_map_.info.resolution);
    ...
    while (ros::ok())
    {
        ...
        social_heatmap_.updateSocialHeatmap(global_2d_map_.info.width, global_2d_map_.info.height, global_2d_map_.info.origin, social_agents_in_fov);

        current_social_heatmap_ = social_heatmap_.getSocialHeatmap();

        social_heatmap_pub_.publish(current_social_heatmap_);
...
bool WorldModeler::agentInFOV(pedsim_msgs::AgentState social_agent)
{
    ...
    double dRobotAgent = std::sqrt(std::pow(social_agent.pose.position.x - robot_odometry_.pose.pose.position.x, 2) +
                                   std::pow(social_agent.pose.position.y - robot_odometry_.pose.pose.position.y, 2));
    ...
    double tethaRobotAgent = atan2((social_agent.pose.position.y - robot_odometry_.pose.pose.position.y),
                                   (social_agent.pose.position.x - robot_odometry_.pose.pose.position.x));
...
pedsim_msgs::AgentStates WorldModeler::socialAgentsInFOV()
{
    ...
    {
        if (agentInFOV(agent_states_.agent_states[i]))
        {
            agentStatesVector.push_back(agent_states_.agent_states[i]);
        }
    }

    agentStatesInFOV.header = agent_states_.header;
...
void WorldModeler::global2DMapCallback(const nav_msgs::OccupancyGridConstPtr &map)
{
    ...
    global_2d_map_.header = map->header;
    global_2d_map_.data = map->data;
    global_2d_map_.info = map->info;
}

void WorldModeler::odometryCallback(const nav_msgs::OdometryConstPtr &odometry)
{
    robot_odometry_.child_frame_id = odometry->child_frame_id;
    robot_odometry_.header = odometry->header;
    robot_odometry_.pose = odometry->pose;
    robot_odometry_.twist = odometry->twist;
}

void WorldModeler::agentStatesCallback(const pedsim_msgs::AgentStatesPtr &social_agents)
{
    agent_states_.agent_states = social_agents->agent_states;
    agent_states_.header = social_agents->header;
}
