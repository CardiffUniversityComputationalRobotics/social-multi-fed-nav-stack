...
class PathFollower:
    ...
    def move_to_point(self, point: Node):
        ...
        #! line of sight linear velocity

        y_error = point.calculate_distance(self.robot_position)
        ...
        for pose in self.following_path:
            path_points_x.append(pose.x)
            path_points_y.append(pose.y)

        np_path_point_x = np.array(path_points_x)
        np_path_point_y = np.array(path_points_y)

        path_line = LineString(np.column_stack((np_path_point_x, np_path_point_y)))

        print(self.robot_position.theta)

        for i in np.arange(0, 2, 0.1):
            los_points_x.append(
                self.robot_position.x + i * 0.1 * math.cos(self.robot_position.theta)
            )
            los_points_y.append(
                self.robot_position.y + i * 0.1 * math.sin(self.robot_position.theta)
            )

        np_los_point_x = np.array(los_points_x)
        np_los_point_y = np.array(los_points_y)

        los_line = LineString(np.column_stack((np_los_point_x, np_los_point_y)))

        intersections = shapely.intersection(path_line, los_line, grid_size=1)

        print(intersections)

        # proj_point =

        vel_msg.linear.x = max(vel_msg.linear.x, self.max_trans_vel)
