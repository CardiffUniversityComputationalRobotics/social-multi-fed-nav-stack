...
void OnlinePlannFramework::planningTimerCallback()
{
    if (goal_available_)
    {
        ...
        if (solved && simple_setup_global_->haveExactSolutionPath())
        {
            ...
            if (simple_setup_global_->haveExactSolutionPath() || distance_to_goal <= goal_radius_)
            {
                ...
                else
                {
                    if (visualize_tree_)
                    {
                        og::PathGeometric empty_path(simple_setup_local_->getSpaceInformation());
                        visualizeRRTLocal(empty_path);
                    }
        ...
        else
        {
            if (visualize_tree_)
            {
                og::PathGeometric empty_path(simple_setup_global_->getSpaceInformation());
