...
private:
    ...
    bool opport_collision_check_, reuse_last_best_solution_, motion_cost_interpolation_, odom_available_,
        ...
        control_active_, global_plan_solving, local_plan_solving;
    ...
    std::vector<const ob::State *> solution_path_states_, local_solution_path_states_;
    ...
    : local_nh_("~"), dynamic_bounds_(false), start_prev_path_proj_(true), goto_action_server_(NULL), control_active_(false)
{
    ...
    global_plan_solving = true;
    local_plan_solving = true;
...
void OnlinePlannFramework::planningTimerCallback()
{
    if (goal_available_ || goal_region_available_)
    {
            ...
            if (simple_setup_global_->haveExactSolutionPath() || distance_to_goal <= goal_radius_)
            {
                ...
                if (solved_local && simple_setup_local_->haveExactSolutionPath())
                {
                    ...
                    if (local_path_states.size() > 0)
                    {
                        ...
                        for (unsigned int i = 0; i < local_path_states.size(); i++)
                        {
                            geometry_msgs::Pose2D p;
                            p.x = local_path_states[i]->as<ob::RealVectorStateSpace::StateType>()->values[0];
                            p.y = local_path_states[i]->as<ob::RealVectorStateSpace::StateType>()->values[1];

                            if (i == (local_path_states.size() - 1))
                            {
                                if (goal_available_)
                                {
                                    ros::Time t;
        ...
        {
            solution_found = false;
            global_plan_solving = false;
        }

        if (!solution_found)
        {
            local_plan_solving = false;
            ...
            if (local_solution_path_states_.size() > 0)
            {
                std::vector<const ob::State *> local_solution_path_states_copy_ = local_solution_path_states_;
...
void OnlinePlannFramework::visualizeRRTFeedback(og::PathGeometric &geopath)
{
    // %Tag(MARKER_INIT)%
    tf::Quaternion orien_quat;
    visualization_msgs::Marker visual_rrt, visual_result_path;
